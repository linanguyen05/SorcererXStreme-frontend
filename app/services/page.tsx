...
// ─── Popup ───────────────────────────────────────────────────────────────────
function ExpertPopup({ expert, anchorRef }: { expert: Expert; anchorRef: React.RefObject<HTMLDivElement> }) {
    ...
    useEffect(() => {
        ...
        // Vertical: try aligning to top of tile, push up if it overflows
        let top = rect.top;
        if (top + POPUP_H > window.innerHeight - 12) top = Math.max(12, window.innerHeight - POPUP_H - 12);
        setPos({ position: 'fixed', top, left, width: W, zIndex: 1000 });
    }, [anchorRef]);
    ...
    return (
        <motion.div
            ...
            style={pos}
            className="flex flex-col rounded-2xl overflow-hidden shadow-2xl shadow-black/80 border border-white/[0.12] bg-[#111118]"
        >
            ...
            <div className="flex-shrink-0 p-4 pb-3 border-b border-white/[0.06]">
                <div className="flex items-start gap-3">
                    ...
                    <div className="relative flex-shrink-0">
                        <div className="absolute -inset-[2px] bg-gradient-to-br from-yellow-400 to-amber-600 rounded-full blur-[3px] opacity-75" />
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={expert.avatar} alt={expert.name}
                            className="relative w-[52px] h-[52px] rounded-full object-cover border-2 border-[#111118]" />
                        {/* Online dot */}
                        {expert.availableOnline && (
                            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-400 border-2 border-[#111118] rounded-full" />
                        )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                            <p className="font-bold text-white text-[15px] leading-tight">{expert.name}</p>
                            <div className="flex-shrink-0 flex items-center gap-1 mt-0.5">
                                <Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" />
                                <span className="text-yellow-300 font-bold text-[13px]">{expert.rating}</span>
                            </div>
                        </div>
                        <p className="text-amber-300/80 text-[11px] mt-0.5 leading-tight">{expert.title}</p>
                        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            {expert.location && (
                                <div className="flex items-center gap-1">
                                    <MapPin className="w-3 h-3 text-gray-500" />
                                    <span className="text-gray-400 text-[11px]">{expert.location}</span>
                                </div>
                            )}
                            {expert.availableOnline ? (
                                <span className="flex items-center gap-1 text-[10px] text-green-300 font-semibold">
                                    <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse inline-block" />
                                    Online ngay
                                </span>
                            ) : (
                                <span className="text-[10px] text-gray-500 font-semibold">Chỉ đặt lịch</span>
                            )}
                            <span className="text-[10px] text-gray-500">{expert.reviewCount} đánh giá</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                    {expert.specialties.map(s => (
                        <span key={s} className={`text-[10px] font-semibold px-2.5 py-[3px] rounded-full border ${PILL[s] ?? 'bg-gray-700/50 text-gray-300 border-gray-600/50'}`}>
                            {s}
                        </span>
                    ))}
                </div>
            ...
            <div className="flex-1 overflow-y-auto" style={{ maxHeight: 320 }}>
                <div className="p-4 space-y-4">
                    ...
                    {/* Bio */}
                    <p className="text-gray-300 text-[12px] leading-relaxed">{expert.bio}</p>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {[
                            ...
                            <div key={label} className="bg-white/[0.05] border border-white/[0.08] rounded-xl p-2.5 text-center">
                                <Icon className="w-3.5 h-3.5 text-yellow-400/80 mx-auto mb-1" />
                                <p className="text-white text-xs font-bold">{val}</p>
                                <p className="text-gray-500 text-[9px] mt-0.5 leading-tight">{label}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Packages */}
                    <div>
                        <p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-gray-500 mb-2">📦 Gói dịch vụ</p>
                        <div className="space-y-1.5">
                            {expert.packages.map(p => (
                                <div key={p.id}
                                    className={`flex items-center justify-between rounded-xl px-3 py-2 ${p.highlight
                                        ? 'bg-gradient-to-r from-yellow-500/12 to-amber-500/8 border border-yellow-500/30'
                                        : 'bg-white/[0.03] border border-white/[0.07]'}`}>
                                    <div className="flex items-center gap-2 min-w-0">
                                        {p.highlight && <Zap className="w-3 h-3 text-yellow-400 flex-shrink-0" />}
                                        <div className="min-w-0">
                                            <p className="text-[12px] font-semibold text-white truncate">{p.name}</p>
                                            <p className="text-[10px] text-gray-500">{p.duration}</p>
                                        </div>
                                    </div>
                                    <span className={`font-bold text-[12px] ml-3 flex-shrink-0 ${p.highlight ? 'text-yellow-300' : 'text-gray-200'}`}>
                                        {formatPrice(p.price)}
                                    </span>
                                </div>
                            ))}
                    ...
                    {expert.social && (
                        <div>
                            <p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-gray-500 mb-2">🌐 Theo dõi</p>
                            <div className="flex flex-wrap gap-2 pointer-events-auto">
                                {SOCIAL_CFG.filter(c => !!expert.social![c.key as keyof SocialLinks]).map(({ key, Icon, label, bg }) => (
                                    <a
                                        ...
                                        rel="noopener noreferrer"
                                        onClick={e => e.stopPropagation()}
                                        className={`flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.05] border border-white/[0.10] text-gray-300 text-[11px] font-medium transition-all duration-200 ${bg}`}
                                        title={label}
                                    >
            ...
            <div className="flex-shrink-0 border-t border-white/[0.08] bg-[#111118] px-4 py-3 flex items-center justify-between gap-3">
                <div>
                    <p className="text-gray-500 text-[9px] uppercase tracking-wider font-bold">Giá từ</p>
                    <p className="text-yellow-300 font-extrabold text-[18px] leading-tight">{formatPrice(minPrice)}</p>
                </div>
                <motion.button
                    whileHover={{ scale: 1.04 }}
                    whileTap={{ scale: 0.97 }}
                    onClick={() => router.push(`/services/${expert.id}`)}
                    className="pointer-events-auto flex items-center gap-2 bg-gradient-to-r from-yellow-400 to-amber-500 text-black text-[12px] font-extrabold px-5 py-2.5 rounded-xl shadow-lg shadow-yellow-900/25 hover:from-yellow-300 hover:to-amber-400 transition-all"
                >
                    Xem & Đặt lịch
    ...
    return (
        ...
        >
            <motion.div
                ...
                onClick={() => router.push(`/services/${expert.id}`)}
                className="group relative aspect-[3/4] rounded-2xl overflow-hidden cursor-pointer border border-white/8 hover:border-yellow-400/50 transition-all duration-300 shadow-md hover:shadow-2xl hover:shadow-yellow-900/15 bg-[#0f0f14]"
            >
                {/* Background */}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="absolute top-3 left-3 right-3 flex items-start justify-between gap-1">
                    {expert.availableOnline ? (
                        ...
                        <div className="bg-black/60 backdrop-blur-sm px-2.5 py-1 rounded-full border border-gray-600/30">
                            <span className="text-gray-400 text-[10px] font-semibold">Booking</span>
                        </div>
                    )}
                    {expert.badges[0] && (
                        <span className="text-[9px] font-extrabold px-2 py-0.5 rounded-full bg-yellow-400/20 border border-yellow-400/40 text-yellow-200 backdrop-blur-sm uppercase tracking-wide">
                            {expert.badges[0]}
                        </span>
                    )}
                </div>

                {/* Avatar — centred, shifts up on hover */}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                    ...
                    {/* Hover reveal */}
                    <div className="overflow-hidden max-h-0 group-hover:max-h-28 transition-all duration-300 mt-0 group-hover:mt-2.5 space-y-2">
                        <div className="flex flex-wrap gap-1 justify-center">
                            {expert.specialties.slice(0, 3).map(s => (
                                <span key={s} className={`text-[9px] font-semibold px-2 py-0.5 rounded-full border ${PILL[s] ?? 'bg-gray-700/60 text-gray-300 border-gray-600/50'}`}>
                                    {s}
                                </span>
                            ))}
                        </div>
                        ...
                        <div className="flex items-center justify-between text-[11px] px-0.5">
                            <span className="text-gray-300">{expert.experience} KN</span>
                            {expert.location && (
                                <div className="flex items-center gap-1">
                                    <MapPin className="w-2.5 h-2.5 text-gray-400" />
                                    <span className="text-gray-300">{expert.location}</span>
                                </div>
                            )}
                        </div>

                        <div className="text-center">
                            <span className="text-yellow-300 font-bold text-sm">Từ {formatPrice(minPrice)}</span>
                        </div>
                    </div>
                </div>
            </motion.div>
            ...
            <AnimatePresence>
                {hovered && <ExpertPopup expert={expert} anchorRef={tileRef as React.RefObject<HTMLDivElement>} />}
            </AnimatePresence>
        </div>
    );
}
                ...
                <div className="max-w-5xl mx-auto px-6 pb-24">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <p className="text-gray-600 text-xs hidden sm:flex items-center gap-1.5">
                            ...
                            Di chuột vào ô để xem chi tiết · Click để đặt lịch
                        </p>
                    </div>
