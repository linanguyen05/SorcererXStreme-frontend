...
// --- VIP Revenue Chart Component (Real API data) ---
function VipRevenueChart({ data }: { data: { totalRevenue: number; activeVipCount: number; estimatedMonthlyRevenue: number; monthlyBreakdown: { month: string; revenue: number; subscribers: number }[] } | null }) {
  ...
  if (!data || data.monthlyBreakdown.length === 0) {
    return (
  ...
  const points = chartData.map((d, index) => {
    const x = padding + (chartData.length > 1 ? (index / (chartData.length - 1)) * chartWidth : chartWidth / 2);
    const rangeBase = maxRevenue - minRevenue * 0.8;
    const y = rangeBase > 0
      ? padding + chartHeight - ((d.revenue - minRevenue * 0.8) / rangeBase) * chartHeight
  ...
  const growthPct = prevMonth > 0 ? (((currentMonth - prevMonth) / prevMonth) * 100).toFixed(1) : '0.0';
    ...
    <div className="bg-[#121214] border border-white/5 rounded-2xl p-6 relative overflow-hidden shadow-2xl group hover:border-red-500/30 transition-all duration-300">
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        <div>
          <span className="text-[10px] font-black uppercase tracking-wider text-red-500 bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded">VIP Revenue</span>
          <h3 className="text-lg font-bold text-white mt-1">Doanh thu VIP Account</h3>
          <p className="text-xs text-gray-500 mt-1">Tổng: <span className="text-white font-bold">{formatVND(data.totalRevenue)}</span> • {data.activeVipCount} VIP hiện tại</p>
        </div>
        <div className="text-right">
          <div className={`flex items-center gap-1 font-bold text-sm px-2.5 py-1 rounded-full border ${Number(growthPct) >= 0 ? 'text-green-400 bg-green-500/10 border-green-500/20' : 'text-red-400 bg-red-500/10 border-red-500/20'}`}>
            <TrendingUp className="w-3.5 h-3.5" />
            <span>{Number(growthPct) >= 0 ? '+' : ''}{growthPct}%</span>
          </div>
        </div>
      </div>
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        try {
          const revenueStats = await adminApi.getVipRevenueStats(token);
  ...
  const activeVipSubscriptions = vipSubscriptions.filter((subscription) => subscription.status === 'active');
        ...
        {activeTab === 'stats' ? (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-[#121214] border border-white/5 rounded-2xl p-5 relative overflow-hidden shadow-xl group hover:border-red-500/30 transition-all duration-300">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Doanh thu VIP</p>
                    <h4 className="text-2xl font-black text-white mt-1.5">{vipRevenueData ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND', notation: 'compact' }).format(vipRevenueData.totalRevenue) : '—'}</h4>
                  </div>
                ...
                <div className="flex items-center gap-1.5 mt-4 text-xs">
                  <span className="text-amber-400 font-bold flex items-center gap-0.5">
                    <Star className="w-3.5 h-3.5" /> {vipTierData ? vipTierData.totalVip : 0} VIP users
                  </span>
                  <span className="text-gray-500 font-medium">× 18.000đ/tháng</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <VipRevenueChart data={vipRevenueData} />
              <ServiceRevenueChart />
            </div>
            ...
            <section className="bg-[#121214] border border-white/5 rounded-2xl p-6 shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between gap-4 mb-5"><div><h3 className="text-lg font-bold text-white flex items-center gap-2"><CreditCard className="w-5 h-5 text-amber-400" /> Giao dịch VIP</h3><p className="text-xs text-gray-500 mt-1">Thông tin lấy trực tiếp từ bảng Subscription và User</p></div><span className="text-xs text-gray-500">{activeVipSubscriptions.length} bản ghi active</span></div>
